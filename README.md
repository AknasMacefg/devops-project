...
- `security-service` — валидатор статического анализа: загружает манифест и пакет, проверяет hash, policy, версию и RSA, затем передаёт артефакт в песочницу.
...
- anti-rollback, чтобы версия была не ниже минимальной и не ниже последней применённой;
...
5. `worker` запускает динамическую проверку в ограниченной среде с лимитами CPU, памяти и времени, а также блокировкой `time.sleep`.
6. Если `worker` вернул `pass`, `security-service` подтверждает обновление для Django.
...
- `malicious_valid` — манифест корректный, но код вредоносный. Обновление блокируется на этапе динамической проверки в `worker`.
...
mTLS между `security-service` и `updater-service` теперь опционален. По умолчанию сервисы работают без TLS, а для включения нужно задать `MTLS_ENABLED=1`.
...
- `Запустить проверку обновления` в Django admin;
- убедитесь, что защита блокирует обновление на статическом этапе и появляются `update_blocked` или `alert`.
...
- `Запустить проверку обновления` в Django admin;
- убедитесь, что обновление блокируется на static-check и/или в sandbox `worker`, после чего появляются `update_blocked` или `alert`.
